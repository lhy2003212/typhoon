...
<div class="main-content">
    <div class="card" style="margin-bottom:10px">
        <div style="display:flex;grid-gap: 20px">
            ...
            <div style="flex:1">
                <div style="font-weight:bold;font-size:24px">{{activity.name}}</div>
                ...
                <div style="display: flex; align-items: center">
                    ...
                    @mouseenter.native="signText='取消报名'" @mouseleave.native="signText='已报名'">已报名</el-button>
                    ...
                    <el-button type="danger" @click="like" v-else>点赞 {{ activity.like }}</el-button>
                    <el-button type="warning" @click="collect" v-if="activity.isCollect">已收藏 {{ activity.collectCount }}</el-button>
                    <el-button type="warning" @click="collect" v-else>收藏 {{ activity.collect }}</el-button>
...
<script>
import Comment from "@/components/CommentFront";
export default {
    name:"ActivityDetail",
    components: {Comment},
    data(){
    ...
    methods:{
        cancel(){
            this.$confirm('确定取消报名吗？','确认取消',{type:"warning"}).then(response=>{
